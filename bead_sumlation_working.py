...
for i in np.arange(len(bead)):
    r[i] = np.sqrt(
        (x - bead[i][0]) ** 2 + (y - bead[i][1]) ** 2 + (z - bead[i][2]) ** 2
    )

bead_volume = (
    (r[0] < (radius)) | (r[1] < radius) | (r[2] < radius) | (r[3] < radius)
).astype(float)
...
for idx, theta in enumerate(angles):
    ...
    rotation_matrix = np.matrix(
        np.array(
            [
                [np.cos(theta), np.sin(theta), 0, t_x],
    ...
    transformed_volume_no_t = ndimage.interpolation.affine_transform(
        bead_volume, rot, order=2, offset=-((offset.T).flatten())
    )
    ...
    for j, element in enumerate(bead):
        bead_pos[j] = rotation_matrix * np.concatenate(
            (np.matrix(bead[j]).T, (np.matrix(1)).T)
        )
    " Projection"
    ...
    transformed_back_projection = ndimage.interpolation.affine_transform(
        back_projection, (rot.I), order=2, offset=-((offset.T).flatten())
    )
    reconstruction_back_projection = (
        transformed_back_projection + reconstruction_back_projection
    )
# %%
