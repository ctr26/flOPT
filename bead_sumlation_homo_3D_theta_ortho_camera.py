...
def draw_fig():
    ...
    plt.savefig('im/drift_beads_homo_noncoplanar_rand' +
                str(angle_idx)+'.pdf', format='pdf')
    # show()
...
for angle_idx, theta in enumerate(angles):
    ...
    transformed_volume = ndimage.interpolation.shift(
        transformed_volume_no_t, trans)
...
#        bead_pos_new[j,angle_idx,:] = np.concatenate([np.array(((rot.I)*np.matrix(bead[j]).T)).flatten(),np.array([1])])
    ...
    reconstruction_back_projection = transformed_back_projection + \
        reconstruction_back_projection
    draw_fig()
