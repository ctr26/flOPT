...
def draw_fig():
    ...
    plt.savefig(
        "im/drift_beads_homo_noncoplanar_rand" + str(angle_idx) + ".pdf", format="pdf"
    )
    # show()
...
for angle_idx, theta in enumerate(angles):
    ...
    transformed_volume_no_t = ndimage.interpolation.affine_transform(
        bead_volume, rot, offset=-((offset.T).flatten())
    )
    ...
    for j, element in enumerate(bead):
        bead_pos[j, angle_idx, :] = (
            rotation_matrix * np.concatenate((np.matrix(bead[j]).T, (np.matrix(1)).T))
        ).flatten()
