...
%% Old
 ...
 lena = rgb2gray(padarray(image,[width/4 width/4]));
...
for i = 1:1:20
    ...
    filtered_irandon            = filtered_irandon - min(min(filtered_irandon));
    filtered_back_projection    = filtered_back_projection - min(min(filtered_back_projection));
    ...
    %Textwidth 437.46112
    imwrite(uint16(filtered_irandon),'filtered_irandon.tiff','WriteMode','append');
    imwrite(uint16(filtered_back_projection),'filtered_back_projection.tiff','WriteMode','append');        
    

    %% Squared difference of pixels

    square_diff_abs_filtered(i) = sum(sum(abs(mat2gray(filtered_back_projection(1:end-1,1:end-1)) - mat2gray(image))));
    square_diff_abs_radon(i)    = sum(sum(abs(mat2gray(filtered_irandon(1:end-1,1:end-1)) - mat2gray(image))));
    %% image correlation
    correlation_filtered(i) =  corr2(filtered_back_projection(1:end-1,1:end-1),image);
    correlation_radon(i)    =  corr2(filtered_irandon(1:end-1,1:end-1),image);

end

%%
plot(square_diff_abs_filtered)
hold on
plot(square_diff_abs_radon)
hold off
%%
plot(correlation_filtered)
hold on
plot(correlation_radon)
hold off
%%
% %%
% f = figure;
