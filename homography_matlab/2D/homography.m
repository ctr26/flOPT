...
current_slice = round(theta*127/(2*pi))+1;
    ...
    rotation_matrix = [cos(theta), sin(theta),t_x; ...
...
square_diff_sum_filtered = sum(sum((filtered_back_projection(1:end-1,1:end-1) - image).^2));
square_diff_sum_radon = sum(sum((filtered_irandon(1:end-1,1:end-1) - image).^2));
