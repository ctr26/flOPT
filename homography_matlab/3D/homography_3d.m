...
radius = 10;
[x y z] = meshgrid(-width/2+1:width/2,-width/2+1:width/2,-width/2+1:width);
...
%%
for theta = 0%linspace(0,2*pi,500)
...
%%
    bead_1_pos = rotation_matrix*([bead_1 1])';
    bead_2_pos = rotation_matrix*([bead_2 1])';
    bead_3_pos = rotation_matrix*([bead_3 1])';
    bead_4_pos = rotation_matrix*([bead_4 1])';
...
%     
    ...
    [transformed_back_projection,cb_translated_ref_back_projection]= imwarp(back_projection,padded_3dref,invert(tform));
...
%     imshow(back_projection,[])
    ...
    subplot(2,3,1);imshow(image,[])%;hold on; plot(bead_1(1)+width/2,bead_1(2)+width/2,'r+');hold off;
    subplot(2,3,2);imshow(transformed_padded,[]);
    hold on;
    plot(bead_1_pos(1)+width,bead_1_pos(2)+width,'r+');
     plot(bead_2_pos(1)+width,bead_2_pos(2)+width,'b+');
     plot(bead_3_pos(1)+width,bead_3_pos(2)+width,'g+');
     plot(bead_4_pos(1)+width,bead_4_pos(2)+width,'y+');
    hold off;
    subplot(2,3,3);imshow(sinugram,[]);
    hold on;
    plot(bead_1_pos(1)+width,1,'r+');
     plot(bead_2_pos(1)+width,1,'b+');
     plot(bead_3_pos(1)+width,1,'g+');
     plot(bead_4_pos(1)+width,1,'y+');
    hold off;
    subplot(2,3,4);
    imshow(back_projection_padded,[])
    subplot(2,3,5);
    imshow(reconstruction_back_projection,[])
...
%%
% cropped_reconstruction = imcrop(flipdim(reconstruction_back_projection,1),[size(reconstruction_back_projection,1)/2-width/2 size(reconstruction_back_projection,2)/2-width/2 width width]);
