...
lena = cv2.normalize(lena.astype('float'),  None, 0,
                     1, cv2.NORM_MINMAX)  # mat2gray
# %%
plt.gray()
plt.subplot(2, 2, 1)
plt.subplot(2, 2, 2)
...
for i in np.arange(len(bead)):
    bead_volume_bool = (bead_volume_bool) | ((np.sqrt((x-bead[i][0])**2 + (
        y-bead[i][1])**2 + (z-bead[i][2])**2)) < (radius))  # Volume image of each bead
...
bead_volume[:, :, round(
    width/2)] = np.maximum(bead_volume[:, :, round(width/2)], lena)
# plt.imshow(bead_volume[:,:,round(width/2)])
#    drawnow
...
# %%
helical_samples = 10
helical_shifts = np.linspace(0, 50, helical_samples)
pc_sum_trans = np.empty((angles.size, helical_samples))
pc_sum_rot = np.empty((angles.size, helical_samples))


drift = np.linspace(0, 1, helical_samples)
alpha_drift = drift*0.25
translation_drift = drift*25
list_of_dfs = {}
...
conditions = {
    "None": {"t_x_helix": drift*0,
             "alpha_helix": drift*0,
             "beta_helix": drift*0,
             "helical_shifts": helical_shifts
             },
    "Tx": {"t_x_helix": translation_drift,
           "alpha_helix": drift*0,
           "beta_helix": drift*0,
           "helical_shifts": translation_drift
           },
    "alpha": {"t_x_helix": drift*0,
              "alpha_helix": alpha_drift,
              "beta_helix": drift*0,
...
for condition_name in conditions:
    ...
    for helicity_idx, helicity in enumerate(helical_shifts):
        for angle_idx, theta in enumerate(angles):
            ...
            alpha_helix = condition["t_x_helix"]
            beta_helix = condition["t_x_helix"]
...
#         t_z = 0
            ...
            #                                               [0               ,0              ,0  ,1]
            #                                               ]))
            alpha = alpha_helix[helicity_idx]
            # alpha_helix[angle_idx]  # 0#theta/100
            ...
            rotation_matrix = np.matrix(np.array([[np.cos(beta)*np.cos(gamma), np.cos(beta)*np.sin(gamma), -np.sin(beta), t_x],
                                                  [np.sin(alpha)*np.sin(beta)*np.cos(gamma)-np.cos(alpha)*np.sin(gamma), np.sin(alpha)*np.sin(
                                                      beta)*np.sin(gamma)+np.cos(alpha)*np.cos(gamma), np.sin(alpha)*np.cos(beta), t_y],
                                                  [np.cos(alpha)*np.sin(beta)*np.cos(gamma)+np.sin(alpha)*np.sin(gamma), np.cos(alpha)*np.sin(
                                                      beta)*np.sin(gamma)-np.sin(alpha)*np.cos(gamma), np.cos(alpha)*np.cos(beta), t_z],
                                                  [0, 0, 0, 1]
                                                  ]))
        ...
        #    plt.imshow(transformed_volume[:,:,round(width/2)])
            ...
            for j, element in enumerate(bead):
                bead_pos[j, angle_idx, :] = (
                    rotation_matrix*np.concatenate((np.matrix(bead[j]).T, (np.matrix(1)).T))).flatten()
        ...
        #        bead_pos_new[j,angle_idx,:] = np.concatenate([np.array(((rot.I)*np.matrix(bead[j]).T)).flatten(),np.array([1])])
            ...
            print(('angle_idx' + str(angle_idx) + 'Rot pc Diff: ' +
                   str(pc_sum_rot[angle_idx, helicity_idx]) + '  Trans pc Diff: ' + str(pc_sum_trans[angle_idx, helicity_idx])))
    ...
    error_types = {"ROT": pc_sum_rot,
                   "TRANS": pc_sum_trans}
    for error_type_key in error_types:

        df = pd.DataFrame(error_types[error_type_key],
                          columns=helical_shifts,
                          index=angles)
        ...
        df_melt = pd.melt(df.reset_index(), id_vars=["angles", "error type", "condition"],
                          var_name="var", value_name="Error")
        df_melt["var"] = pd.to_numeric(df_melt["var"], errors='coerce').\
            round(decimals=2)
...
# sns.violinplot(x=condition_name, y="Error", data=df_melt)
# plt.xticks(rotation=90)
# plt.tight_layout()
g = sns.catplot(x="var", y="Error",
                row="condition", col="error type",
                data=full_df, kind="box", sharey=False, sharex=False)
g.set_xticklabels(rotation=90, horizontalalignment='right')
# %%

# %%

#    # io.savemat('helicity_H_recovered_both.mat', mdict={'pc_sum_rot': pc_sum_rot,'pc_sum_trans': pc_sum_trans})
# # %% Plotting

# # with plt.style.context(('default','/Users/craggles/OneDrive - University Of Cambridge/Programming/Python/stylesheets/thesis')):
# fig2 = plt.figure()
# plt.boxplot(pc_sum_trans[:, :],
#             positions=np.round(helical_shifts, 1)/100)
# plt.xlabel('Helical shift /rad')
...
# plt.tight_layout()
# #plt.savefig('pc_sum_trans_alpha.pdf', format='pdf')
# # %%
# # with plt.style.context(('default','/Users/craggles/OneDrive - University Of Cambridge/Programming/Python/stylesheets/thesis')):
# fig3 = plt.figure()
# plt.boxplot(pc_sum_rot[:, :], positions=np.round(
#     helical_shifts, 1)/100)
# plt.xlabel('Helical shift /rad')
# plt.yticks(fontname='Helvetica')
# plt.ylabel('% error')
# plt.xticks(fontname='Helvetica')
# plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))
# plt.tight_layout()
# #plt.savefig('pc_sum_rot_alpha.pdf', format='pdf')
...
df = pd.DataFrame(pc_sum_trans, columns=helical_shifts, index=angles)
df.index.names = ['angles']
# df.columns.names = ['Tx']
df
# %%
...
df_melt = pd.melt(df.reset_index(), id_vars=["angles"],
                  var_name='Tx', value_name="Error")
df_melt["Tx"] = pd.to_numeric(df_melt["Tx"], errors='coerce').\
    round(decimals=2)
# %%
sns.violinplot(x="Tx", y="Error", data=df_melt)
plt.xticks(rotation=90)
plt.tight_layout()
# %%
